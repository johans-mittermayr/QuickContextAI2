...
window.bubbleAutoCloseTimer = window.bubbleAutoCloseTimer || null;
...
function renderBubble({ text, error = false, theme = 'auto', autoClose = 10000 }) {
  ...
  document.getElementById('quickcontext-close').onclick = () => {
    if (bubbleAutoCloseTimer) clearTimeout(bubbleAutoCloseTimer);
    bubble.remove();
  };
  ...
  // ✨ Auto-close logic
  if (window.bubbleAutoCloseTimer) clearTimeout(window.bubbleAutoCloseTimer);
  if (autoClose > 0) {
    window.bubbleAutoCloseTimer = setTimeout(() => {
